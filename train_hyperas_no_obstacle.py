...
def create_model_is_tansyo(X_train, Y_train, X_test, Y_test):
    ...
    history = model.fit(
        train_data, train_label, validation_data=(val_data, val_label), epochs=30, batch_size=256, callbacks=callbacks
    )
...
def hyperas_learn(target_name):
    ...
    if target_name == "is_tansyo":
        best_run, best_model = optim.minimize(
            model=create_model_is_tansyo, data=prepare_data_is_tansyo, algo=tpe.suggest, max_evals=15, trials=Trials()
        )
        _, _, X_test, Y_test = prepare_data_is_tansyo()
    elif target_name == "is_hukusyo":
        best_run, best_model = optim.minimize(
            model=create_model_is_hukusyo, data=prepare_data_is_hukusyo, algo=tpe.suggest, max_evals=15, trials=Trials()
        )
