    ...
    model = tf.keras.Sequential(
        [
            tf.keras.layers.Dense(
                300, kernel_regularizer=tf.keras.regularizers.l2(0.001), activation=tf.nn.relu, input_dim=df_columns_len
            ),
...
def train_model(
    ...
    callbacks = [
        ...
        ModelCheckpoint("model/{}_{}_model.h5".format(OWN_FILE_NAME, target_name), save_best_only=True),
    ]
    callbacks.append(CSVLogger("model/{}_{}_history.csv".format(OWN_FILE_NAME, target_name)))
    model = build_model(train_data.shape[1])
    history = model.fit(
        train_data, train_label, validation_data=(val_data, val_label), epochs=100, batch_size=256, callbacks=callbacks
    )
